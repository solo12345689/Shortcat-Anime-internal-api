package Cf;

import java.util.Iterator;
import java.util.NoSuchElementException;
import je.InterfaceC5371a;
import kotlin.jvm.functions.Function1;
import kotlin.jvm.internal.AbstractC5504s;

/* JADX INFO: compiled from: r8-map-id-a27a03ecaa04c3f144355334beb55c9cbfc36dda79e554423bceb24cf939fcb7 */
/* JADX INFO: loaded from: classes5.dex */
public final class y implements i {

    /* JADX INFO: renamed from: a, reason: collision with root package name */
    private final i f3130a;

    /* JADX INFO: renamed from: b, reason: collision with root package name */
    private final Function1 f3131b;

    /* JADX INFO: compiled from: r8-map-id-a27a03ecaa04c3f144355334beb55c9cbfc36dda79e554423bceb24cf939fcb7 */
    public static final class a implements Iterator, InterfaceC5371a {

        /* JADX INFO: renamed from: a, reason: collision with root package name */
        private final Iterator f3132a;

        /* JADX INFO: renamed from: b, reason: collision with root package name */
        private int f3133b = -1;

        /* JADX INFO: renamed from: c, reason: collision with root package name */
        private Object f3134c;

        a() {
            this.f3132a = y.this.f3130a.iterator();
        }

        private final void b() {
            if (this.f3132a.hasNext()) {
                Object next = this.f3132a.next();
                if (((Boolean) y.this.f3131b.invoke(next)).booleanValue()) {
                    this.f3133b = 1;
                    this.f3134c = next;
                    return;
                }
            }
            this.f3133b = 0;
        }

        @Override // java.util.Iterator
        public boolean hasNext() {
            if (this.f3133b == -1) {
                b();
            }
            return this.f3133b == 1;
        }

        @Override // java.util.Iterator
        public Object next() {
            if (this.f3133b == -1) {
                b();
            }
            if (this.f3133b == 0) {
                throw new NoSuchElementException();
            }
            Object obj = this.f3134c;
            this.f3134c = null;
            this.f3133b = -1;
            return obj;
        }

        @Override // java.util.Iterator
        public void remove() {
            throw new UnsupportedOperationException("Operation is not supported for read-only collection");
        }
    }

    public y(i sequence, Function1 predicate) {
        AbstractC5504s.h(sequence, "sequence");
        AbstractC5504s.h(predicate, "predicate");
        this.f3130a = sequence;
        this.f3131b = predicate;
    }

    @Override // Cf.i
    public Iterator iterator() {
        return new a();
    }
}
